...
project_image = ctk.CTkImage(light_image=Image.open(r"icons\system\projectType.png"),
                             dark_image=Image.open(r"icons\system\projectType.png"), size=(16,16))
...
class TabView:
    def __init__(self, master, width: int = 370, height: int = 500, corner_radius=0, border_width=1,
        ...
        self.explnLabel = ctk.CTkLabel(self.solutionExplorerFrame, text=f"  Solution: {number_of_files} files found",
                                       font=("Segoe UI", 12), fg_color=["#F5F5F5", "#1E1E1E"], width=width-2,
                                       image=project_image, compound="left", height=18, justify="left", anchor="w",
                                       text_color=["#1E1E1E", "#F5F5F5"])
        self.explnLabel.place(x=5, y=5)

        self.gadgetsFrame = ctk.CTkFrame(self.solutionExplorerFrame, width=width, height=26,
                                         fg_color=["#F5F5F5", "#1E1E1E"], border_width=1, corner_radius=0,
                                         border_color=["#C8C8C8", "#444444"])
        self.gadgetsFrame.place(x=0, y=26)

        self.newFileBtn = uniwidgets.SmallButton(self.gadgetsFrame, image_path=r"icons\system\newnew.png")
        self.newFileBtn.place(x=2, y=2)
        self.newDirBtn = uniwidgets.SmallButton(self.gadgetsFrame, image_path=r"icons\system\newDir.png")
        self.newDirBtn.place(x=26, y=2)
        self.refreshBtn = uniwidgets.SmallButton(self.gadgetsFrame, image_path=r"icons\system\refreshDir.png")
        self.refreshBtn.place(x=50, y=2)
        self.collapseBtn = uniwidgets.SmallButton(self.gadgetsFrame, image_path=r"icons\system\collapse.png")
        self.collapseBtn.place(x=74, y=2)

        self.searchBar = ctk.CTkEntry(self.gadgetsFrame, fg_color=["#F5F5F5", "#1E1E1E"],
                                      border_color=["#C8C8C8", "#444444"], border_width=1, height=20,
                                      font=("Segoe UI", 12), width=250, corner_radius=0,
        ...
        self.slnExplrBtn = ctk.CTkButton(master=self.tabChanger, width=50, height=(height/17),
                                         text="Solution Explorer ", text_color=["#262626", "#E3E3E3"],
                                         corner_radius=6, hover=False, border_width=0,
                                         font=("Segoe UI", 12), fg_color=["#F5F5F5", "#1E1E1E"])
        self.slnExplrBtn.place(x=8, y=-5)
        self.propertiesBtn = ctk.CTkButton(master=self.tabChanger, width=50, height=(height/17),
                                           text="      Properties      ", text_color=["#262626", "#E3E3E3"],
                                           corner_radius=6, hover=False, border_width=0,
                                           font=("Segoe UI", 12), fg_color=["#F5F5F5", "#1E1E1E"])
        self.propertiesBtn.place(x=118, y=-5)
        self.gitReposBtn = ctk.CTkButton(master=self.tabChanger, width=50, height=(height/17),
                                         text=" Git Repositories ", text_color=["#262626", "#E3E3E3"],
                                         corner_radius=6, hover=False, border_width=0,
                                         font=("Segoe UI", 12), fg_color=["#F5F5F5", "#1E1E1E"])
