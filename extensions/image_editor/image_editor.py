...
class ImageEditor(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        self.rotateBtn = ctk.CTkButton(
            self.leftFrame, width=46, height=46, text=""
        ).pack(padx=2, pady=(4, 0))
        self.zoomInBtn = ctk.CTkButton(
            self.leftFrame, width=46, height=46, text=""
        ).pack(padx=2, pady=2)
        self.zoomoutBtn = ctk.CTkButton(
            self.leftFrame, width=46, height=46, text=""
        ).pack(padx=2, pady=2)
        self.invertXBtn = ctk.CTkButton(
            self.leftFrame, width=46, height=46, text=""
        ).pack(padx=2, pady=2)
        self.invertYBtn = ctk.CTkButton(
            self.leftFrame, width=46, height=46, text=""
        ).pack(padx=2, pady=2)
        self.brightnessBar = ctk.CTkSlider(self.leftFrame, orientation="vertical").pack(
            padx=5, pady=5
        )
