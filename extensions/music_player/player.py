...
class mainApp(tk.Toplevel):
    def __init__(self, mode: str, icon: str | None = None, master=None, music_path=None):
        ...
        if icon and isinstance(icon, str):
            ...
            pil_icon = Image.open(r"Extensions\music_player\music.png").resize((128, 128))
        ...
        self.label1 = tk.Label(self.status, border=0, text="Now Playing: ", height=24,
                               background=colors_scheme[0], justify="left", anchor="w",
                               font=("Segoe UI", 9), foreground=colors_scheme[3])
        self.label1.pack(side='left', padx=5, pady=3)

        self.pathLabel = tk.Label(self.status, border=0, text=f"{self.music_path}", height=24,
                                  background=colors_scheme[0], justify="left", anchor="w",
                                  font=("Segoe UI", 9), foreground=colors_scheme[3])
        self.pathLabel.pack(side='left', padx=5, pady=3)

        self.playStopBtn = tk.Button(self.status, overrelief="flat", border=1,
                                     text="▶", background=colors_scheme[1],
                                     foreground=colors_scheme[3], font=("Segoe UI",12),
        ...
        self.icon_label = tk.Label(self.gadgets, border=0, background=colors_scheme[1],
                                   image=self.tk_icon, text="", justify='center',
                                   anchor="center")
