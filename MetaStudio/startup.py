...
def _load_image_cached(path, size=None):
    key = (path, tuple(size) if size else None)
    if key not in _image_cache:
        _image_cache[key] = Image.open(path)
    return _image_cache[key]


def _preload_images():
    preload_paths = [
        (r"icons\logos\logo.png", (700, 400)),
        (r"icons\system\newvar.png", (27, 27)),
        (r"icons\system\folder.ico", (27, 27)),
    ]
    for path, size in preload_paths:
        if os.path.exists(path):
            _load_image_cached(path, size)
...
class Initialize:
    ...
    def start_loading_screen(self):
        self.splash.after(500, self._check_load_complete)
        self.splash.mainloop()

    def _check_load_complete(self):
        if len(_image_cache) < 2:
            self.splash.after(200, self._check_load_complete)
        else:
            self.start_main_app()
