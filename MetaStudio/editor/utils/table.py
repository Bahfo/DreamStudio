...
class CTkTable(customtkinter.CTkFrame):
            ...
            for j in range(self.columns):
                ...
                if self.write:
                    ...
                    self.frame[i, j] = customtkinter.CTkEntry(
                        self.inside_frame, font=self.font, corner_radius=0, **args
                    )
                    if value is None:
                        value = " "
                    ...
                    self.frame[i, j] = customtkinter.CTkButton(
                        ...
                        border_width=0,
                        command=(
                            (lambda e=self.data[i, j]: self.command(e))
                            if self.command
                            else None
                        ),
                        **args
    ...
    def dynamic_hover(self, frame, i, j):
        ...
        frame.bind(
            "<Enter>",
            lambda e, x=i, y=j, color=hover_corners, fg=hv: self.frame[x, y].configure(
                background_corner_colors=color, fg_color=fg
            ),
        )
        frame.bind(
            "<Leave>",
            lambda e, x=i, y=j, color=corners, fg=fg: self.frame[x, y].configure(
                background_corner_colors=color, fg_color=fg
            ),
        )
    ...
    def select(self, row, column):
        ...
        self.frame[row, column].configure(
            background_corner_colors=hover_corners, fg_color=self.hover_color
        )
    ...
    def get_selected_row(self):
        ...
        for i in range(self.rows):
            if self.frame[i, 0].cget("fg_color") == self.hover_color:
                ...
                break
        selected_row_data = (
            self.get_row(selected_row_index) if selected_row_index is not None else None
        )
    ...
    def get_selected_column(self):
        ...
        selected_column_data = (
            self.get_column(selected_column_index)
