...
class Terminal:
    def __init__(self):
        ...
        self.error_0 = (
            "bad command, perhaps you check list of commands? type showcommands"
        )
        ...
        self.terminal_textbox.insert(
            "0.0", "DreamStudio Command Windows \nCOPYRIGHT 2026 EX Technologies\n"
        )
    ...
    def on_enter(self, event=None):
        ...
        threading.Thread(
            target=self.command_execution,
            args=(text, self.terminal_textbox),
            daemon=True,
        ).start()
        self.insert_prompt()
        return "break"
    ...
    def command_execution(self, user_input, screen_widget):
        ...
        if len(parts) == 4:
            ...
            parameter_3 = parts[
                3
            ]  # Source Language / Or Destination Path / Or number of lines to display
            ...
            if command_identifier == "dispfirstlines":
                ...
                if not os.path.exists(full_path):
                    screen_widget.insert(
                        ctk.END,
                        f"File '{parameter_1}' not found in '{parameter_2}' {self.error_1}\n",
                    )
                else:
                    try:
                        ...
                        screen_widget.insert(
                            ctk.END, f"Error showing file contents: {e}\n"
                        )
            ...
            # RENAME
                ...
                if not os.path.exists(full_path):
                    ...
                    try:
                        ...
                        screen_widget.insert(
                            ctk.END,
                            f"File '{parameter_1}' name is changed to '{parameter_3}'\n",
                        )
                    except Exception as e:
                ...
                if not os.path.exists(full_path):
                    screen_widget.insert(
                        ctk.END, f"File '{parameter_1}' not found in '{parameter_2}'\n"
                    )
                else:
                    try:
                        ...
                        if parameter_3 == "wordsum":
                            ...
                            screen_widget.insert(
                                ctk.END,
                                f"Number of words in {parameter_1} is {word_count}\n",
                            )
                            ...
                            screen_widget.insert(
                                ctk.END,
                                f"Number of characters in {parameter_1} is {char_count} with spaces\n",
                            )
                            ...
                            screen_widget.insert(
                                ctk.END,
                                f"Number of characters in {parameter_1} is {char_count_nosp} without spaces\n",
                            )
                            ...
                            screen_widget.insert(
                                ctk.END,
                                f"Number of lines in {parameter_1} is {line_count}\n",
                            )
                ...
                try:
                    ...
                    if not os.path.exists(full_path_delete_ws):
                        ...
                        if not os.listdir(full_path_delete_ws):  # folder is empty
                            ...
                            user_input = (
                                self.terminal_textbox.get("end-2c linestart", "end-1c")
                                .strip()
                                .lower()
                            )
                            if user_input == "y":
                ...
                try:
                    for root, dirs, files in os.walk(parameter_1):
                        if parameter_2 in files:
                            screen_widget.insert(
                                ctk.END, os.path.join(root, parameter_2)
                            )
