...
def draw_top_bars(active_tab):
    ...
    print(
        terminal.move_xy(0, 0)
        + terminal.on_black
        + terminal.white
        + title_text.center(terminal.width),
        end="",
    )
...
def draw_status_bar():
    ...
    for key, desc in segments:
        status_bar_line += (
            WINDOW_BG + KEY_HIGHLIGHT_RED + key + WINDOW_BG + terminal.black + desc
        )
    status_bar_line += WINDOW_BG + (" " * padding_length)
    print(
        terminal.move_xy(0, terminal.height - 1) + status_bar_line, end="", flush=True
    )
...
def draw_progress_modal(title, message, current, total, width=80):
    ...
    progress_bar = (
        terminal.cyan + "█" * filled_units + terminal.white + "░" * empty_units
    )

    for i in range(height):
        print(
            terminal.move_xy(start_x + 2, start_y + i + 1)
            + terminal.on_black
            + " " * width,
            end="",
        )
    ...
    print(
        terminal.move_xy(start_x + 3, start_y + 1)
        + WINDOW_BG
        + terminal.black
        + terminal.bold(title),
        end="",
    ...
    print(
        terminal.move_xy(start_x + 4, start_y + 3)
        + WINDOW_BG
        + terminal.black
        + f"{message:<{width-8}}",
        end="",
    ...
    print(
        terminal.move_xy(start_x + 4, start_y + 5)
        + WINDOW_BG
        + terminal.black
        + f"[ {progress_bar}{terminal.black} ] {percent:3}%",
        end="",
    )
...
def draw_window_frame(title, num_lines, width=100):
    ...
    print(
        terminal.move_xy(start_x + 3, start_y + 2) + WINDOW_BG + terminal.black,
        "Welcome to DS Installer - DreamStudio IDE Interactive Installation",
    )
    print(
        terminal.move_xy(start_x + 3, start_y + 3) + WINDOW_BG + terminal.black,
    ...
    print(
        terminal.move_xy(inner_x, inner_y)
        + WINDOW_BG
        + terminal.black
        + "┌"
        + "─" * (inner_w - 2)
    ...
    for i in range(1, inner_height + 1):
        print(
            terminal.move_xy(inner_x, inner_y + i)
            + WINDOW_BG
            + terminal.black
            + "│"
            + " " * (inner_w - 2)
    ...
    print(
        terminal.move_xy(inner_x, inner_y + inner_height + 1)
        + WINDOW_BG
        + terminal.black
        + "└"
        + "─" * (inner_w - 2)
    ...
    print(
        terminal.move_xy(start_x + 2, start_y + height - 3)
        + WINDOW_BG
        + terminal.black
        + "─" * (width - 4),
        end="",
    ...
    cancel_btn = " <Cancel> "
    print(
        terminal.move_xy(start_x + (width // 2) - 13, start_y + height - 2)
        + terminal.on_blue
        + terminal.yellow
        + ok_btn,
        end="",
    ...
    print(
        terminal.move_xy(start_x + (width // 2) + 2, start_y + height - 2)
        + WINDOW_BG
        + terminal.black
        + cancel_btn,
        end="",
...
def draw_options_only(options, selected_idx, start_x, start_y, content_width):
    ...
    for idx, (name, desc) in enumerate(options):
        ...
        if idx == selected_idx:
            ...
            print(
                terminal.move_xy(start_x, y_pos)
                + SELECT_BG
                + SELECT_TEXT
                + padded_line,
                end="",
            ...
            print(
                terminal.move_xy(start_x, y_pos) + WINDOW_BG + " " * content_width,
                end="",
            )
            ...
            print(
                terminal.move_xy(start_x + 2, y_pos)
                + WINDOW_BG
                + TEXT_MAIN_BLUE
                + f"{rest_of_name:<18}",
                end="",
            ...
            print(
                terminal.move_xy(start_x + 22, y_pos)
                + WINDOW_BG
                + TEXT_DESC_BLACK
                + desc,
                end="",
...
def draw_static_info_pane(title, lines, width=86):
    ...
    for idx, line in enumerate(lines):
        print(
            terminal.move_xy(inner_x + 2, inner_y + 1 + idx)
            + WINDOW_BG
            + TEXT_DESC_BLACK
            + f"{line:<{inner_w-4}}",
            end="",
...
def show_interactive_less_menu(title, lines, width=80):
    ...
    for idx, line in enumerate(lines):
        print(
            terminal.move_xy(start_x + 4, start_y + 3 + idx)
            + WINDOW_BG
            + terminal.black
            + line,
            end="",
    ...
    ok_btn = "  < OK >  "
    print(
        terminal.move_xy(start_x + (width // 2) - 5, start_y + height - 2)
        + terminal.on_blue
        + terminal.yellow
        + ok_btn,
        end="",
...
options = [
    ...
    (
        "Aquire Requirements",
        "Install all requirements for DreamStudio (Manual Installations)",
    ),
...
with terminal.fullscreen(), terminal.cbreak(), terminal.hidden_cursor():
    ...
    draw_background()
    draw_top_bars(active_tab)
    start_x, start_y, content_width = draw_window_frame(
        "DSInstaller Options Manager", len(options)
    )
    title_text = " DSInstaller v1.0.0 - DreamStudio Setup "
    draw_status_bar()
    ...
    while True:
        if active_tab == 0:
            ...
            draw_static_info_pane(
                "System Information & Licensing Overview",
                aggregated_information,
                width=100,
            )
        ...
        if key.name == "KEY_LEFT":
            ...
            if selected_option == 0:
                ...
                for i in range(101):
                    draw_progress_modal(
                        "Upgrade Status",
                        "Searching and fetching updates...",
                        current=i,
                        total=100,
                    )
                    time.sleep(0.03)
