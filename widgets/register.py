...
def agreement_registeration():
    ...
    frame_1 = ctk.CTkFrame(
        window, bg_color="#1e1e1e", width=550, height=440, fg_color="#303030"
    )
    ...
    continue_button = ctk.CTkButton(
        ...
        corner_radius=5,
        state=ctk.DISABLED,
        command=lambda: frame_2.tkraise(),
    )
    ...
    new_size = (300, 300)
    frames_loading = [
        ImageTk.PhotoImage(frame.copy().resize(new_size))
    ...
    frames_set = [
        ImageTk.PhotoImage(frame.copy().resize(new_size))
        for frame in ImageSequence.Iterator(all_set_image)
    ]
