...
class AddMacrosTab(ctk.CTkToplevel):
    def __init__(self, main_window, master, text_editor, switcher, status_button=None, *args, **kwargs):
        ...
        # Right Components
        self.title_label = ctk.CTkLabel(self, text="Add a Code Snippet", font=("Segoe UI",20),
                                        text_color="#C4C4C4" if self.mode=='dark' else "#1E1E1E",
                                        corner_radius=0)
        self.title_label.place(x=220, y=8)

        self.add_button = ctk.CTkButton(self.left_frame, text="Add Macro", font=("Segoe UI",12),
                                        fg_color="#005393",hover_color="#003660",
                                        corner_radius=5,border_width=0, command=self.addMacroFn)
        
        self.description_label = ctk.CTkLabel(self, text="", font=("Segoe UI",12),
                                        text_color="#C4C4C4" if self.mode=='dark' else "#1E1E1E",
                                        corner_radius=0,anchor='w',justify="left")
        
        self.image = ctk.CTkLabel(self, text="", corner_radius=0)
        self.image.place(x=220, y=40)

        self.textbox = ctk.CTkTextbox(self, width=410, height=200, font=("Consolas",15))
        self.textbox.configure(state=ctk.DISABLED)
        self.textbox.place(x=220,y=80)

        self.classCode = textwrap.dedent("""class Macro:
    ...
    def function(self):
        return
""")
        self.mainCode = textwrap.dedent("""if __name__ == '__main__':
    run_program()
    return
""")
        self.tryExceptCode = textwrap.dedent("""try:
    if var1:
        runCommand_1()
    else:
        runCommand_2()
...
""")
        self.functionCode = textwrap.dedent("""def function:
    return
""")
        self.headCode = textwrap.dedent("""'''Author:
Date:
Description:'''
""")
        self.tailCode = textwrap.dedent("""'''COPYRIGHT 20XX BY COMPANY NAME
ALL RIGHTS RESERVED'''
""")
...
class ConfigureSyntax(ctk.CTkToplevel):
    def __init__(self, main_window, master, text_editor, status_button=None, *args, **kwargs):
        # Correct usage of super()
        super().__init__(master, *args, **kwargs)
        ...
        # Right Components
        self.title_label = ctk.CTkLabel(self, text="Configure Theme", font=("Segoe UI",20),
                                        text_color="#C4C4C4" if self.mode=='dark' else "#1E1E1E",
                                        corner_radius=0)
        self.title_label.place(x=220, y=8)

        self.add_button = ctk.CTkButton(self.left_frame, text="Change Theme", font=("Segoe UI",12),
                                        fg_color="#005393",hover_color="#003660",
        ...
        self.textbox = ctk.CTkTextbox(self, width=410, height=260, font=("Consolas",15))
        self.textbox.configure(state=ctk.DISABLED)
